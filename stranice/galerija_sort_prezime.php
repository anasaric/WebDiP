...
$baza = new baza();
$baza->spojiDB();
$query = "SELECT DISTINCT * FROM `galerija_slika` INNER JOIN `prijava` ON `galerija_slika`.korisnik_korisnik_id=`prijava`.korisnik_id AND `galerija_slika`.utrka_id=`prijava`.utrka_id INNER JOIN `korisnik` ON `korisnik`.korisnik_id=`galerija_slika`.korisnik_korisnik_id ORDER BY prezime";
...
$upit = "sortiranje podataka po prezmenu korisnika";
